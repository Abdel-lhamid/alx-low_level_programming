...
/**
 * handle_error - prints error messages and exits with exit value
 * @error: num is either exit value or file descriptor
 * @s: str is a name, either of the two filenames
 * @fd: file descriptor
 * Return: 0 on success
 **/
int handle_error(int error, char *s, int fd)
{
	switch (error)
	{
	case 97:
...
int main(int argc, char *argv[])
{
	...
	if (close(fd_2) == -1)
		handle_error(100, NULL, fd_2);
	if (close(fd_1) == -1)
		handle_error(100, NULL, fd_1);
